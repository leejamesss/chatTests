...
const globleQuestion = ref({
  question: "以下关于数据库知识点的叙述中，不正确的是（）。",
  A: "主键是表中用来唯一标识每条记录的列",
  B: "外键用于建立表与表之间的关系",
  C: "索引是一种数据结构，用于提高数据的插入速度",
  D: "事务具有原子性、一致性、隔离性和持久性（ACID属性）",
  rightIndex: 2,
  analyze:
    "在数据库知识点中，主键是用来唯一标识表中每条记录的列，它确保每条记录都有唯一的标识。外键用于建立表与表之间的关系，确保数据的完整性和一致性。索引是一种数据结构，用于提高数据的检索速度，而非插入速度。事务具有原子性、一致性、隔离性和持久性（ACID属性），它们是事务的特性，而非索引的作用。因此，选项 C 描述是不正确的。",
});
// 题目标题累加器
var accumulateQuestion = "";
// 返回出题信息
var questionInfo = "";
var learningScope = "数据库";
var learningArea =
  "SQL、数据库系统、关系型数据库、数据库管理系统、实体-关系模型、范式、视图、事务、索引、表、主键、外键";
...
const jsonContent = `{
  ...
  "D": "事务具有原子性、一致性、隔离性和持久性（ACID属性）",
  "rightIndex": 2,
  "analyze": "在数据库知识点中，主键是用来唯一标识表中每条记录的列，它确保每条记录都有唯一的标识。外键用于建立表与表之间的关系，确保数据的完整性和一致性。索引是一种数据结构，用于提高数据的检索速度，而非插入速度。事务具有原子性、一致性、隔离性和持久性（ACID属性），它们是事务的特性，而非索引的作用。因此，选项 C 描述是不正确的。"
}`;
  ...
  const tmpMessageList = ref<ChatMessage[]>([
    {
      ...
      content: `你是一个备考专家，需要为用户提供出单选题的服务，并排除用###符号分割的题干。
          ...
          举例输出JSON 如下：
          {
            ...
            "D": "事务具有原子性、一致性、隔离性和持久性（ACID属性）",
            "rightIndex": 2,
            "analyze": "在数据库知识点中，主键是用来唯一标识表中每条记录的列，它确保每条记录都有唯一的标识。外键用于建立表与表之间的关系，确保数据的完整性和一致性。索引是一种数据结构，用于提高数据的检索速度，而非插入速度。事务具有原子性、一致性、隔离性和持久性（ACID属性），它们是事务的特性。因此，选项 C 描述是不正确的。"
          }`,
    },
    {
      role: "user",
      content:
        `请随机给出一个数据库相关的题目，范围是SQL、数据库系统、关系型数据库、数据库管理系统、实体-关系模型、范式、视图、事务、索引、表、主键、外键。排除 ###` +
        accumulateQuestion +
        `###`,
    },
  ]);
